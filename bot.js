...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.commandName == "infos") {
    ...
    if (!user) {
      await interaction.editReply(`User with login: ${userName} does not exist`);
      return false;
    } else {
      const stream = await user.getStream();
      let embed = new EmbedBuilder().setColor('Purple')
    ...
    if (!user) {
      ...
      if (channeldataobject !== undefined) {
        if (channeldataobject.streams?.find(s => s.user.id == user.id) == undefined) {
          ...
          await interaction.editReply(`You will now recieve a notification in this channel when ${user.displayName} goes live!`)
          console.log(`followed ${user.displayName} in channel: ${channeldataobject.chanId}`)
        } else {
          await interaction.editReply(`You're already following ${user.displayName} in this channel!`)
        }
      } else {
    ...
    if (!user) {
      ...
      if (channeldataobject !== undefined) {
        if (channeldataobject.streams?.find(s => s.user.id == user.id) !== undefined) {
          ...
          await interaction.editReply(`You will not recieve a notification in this channel when ${user.displayName} goes live anymore!`)
          console.log(`unfollowed ${user.displayName} in channel: ${channeldataobject.chanId}`)
        } else {
          await interaction.editReply(`You're already not following ${user.displayName} in this channel`)
        }
      } else {
        await interaction.editReply(`You're already not following ${user.displayName} in this channel`)
      }
